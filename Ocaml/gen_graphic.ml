...
(*let _ = print_string (gen_graphic (List.map (function x -> 7.*.(float_of_int
 * x) +. 5.5) (calc_listn 3))  (List.map (function x -> 4.*.(float_of_int x))
 * (calc_listn 4)))*)
let _ = print_string (gen_graphic [0.; 1.; 1.7] [0.2;1.2;2.2]) 
